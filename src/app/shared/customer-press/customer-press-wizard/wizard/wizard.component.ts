...
@Component({
  ...
  `<div class="card">
    ...
    <div class="wizard-cont" [hidden]="isCompleted">
        <button type="button" class="btn   btn-primary float-left" (click)="previous()" [hidden]="!hasPrevStep || !activeStep.showPrev">{{'CUSTOMER_PRESS.previous' | translate}}</button>
        <button type="button" class="btn   btn-primary float-right" (click)="next()" [disabled]="!activeStep.isValid" [hidden]="!hasNextStep || !activeStep.showNext">{{'CUSTOMER_PRESS.next' | translate}}</button>
        <button type="button" class="btn  btn-primary float-right" (click)="complete()" [disabled]="!activeStep.isValid" [hidden]="hasNextStep">{{'CUSTOMER_PRESS.done' | translate}}</button>
    </div>
  </div>`
  ,
  styles: [
    '.wizard-cont{ margin: 0% 5% 3% 5%;}',
    '.card { height: 100%; }',
